...
We follow the [Conventional Commits](https://www.conventionalcommits.org/) specification for our commit messages. This helps us maintain a clean and readable git history, and enables automated changelog generation.
...
The scope is optional and can be used to provide more context about the changes being made. However, it is strongly recommended to use the scope to make the commit message more readable and easier to understand. In LifeForge, the scope is usually the codebase or module being modified. For example, `client`, `server`, `ui`, `shared`, `docs`, 'forgeCLI', etc.

### Examples

```plaintext
feat(client,server): add QR code login functionality

fix(ui): resolve widget overflow issue on mobile

docs(server): update endpoint documentation for user routes

refactor(client): simplify module loading logic

feat(shared): add `useDivSize` hook for getting div size
```

<Alert type="tip">
Keep the first line (subject) under 72 characters. Use the body to explain what and why, not how.
</Alert>

<Alert type="note">
If your commit introduces a breaking change, add `BREAKING CHANGE:` in the footer or append `!` after the type/scope, e.g.

```plaintext
feat(server)!: remove deprecated endpoints
```
</Alert>

### AI Commit Message Generation

If you are using AI to generate commit messages, most editors allow you to use custom prompts to guide the AI tool to generate content that aligns with our project's standards and guidelines.

```plaintext
Generate a commit message following Conventional Commits: <Alert type>(<scope>): <description>

Types: feat, fix, docs, style, refactor, perf, test, build, ci, chore, revert
Scope: client, server, ui, shared, docs, forgeCLI, etc. (use comma for multiple, e.g., client,server)

Rules:
- Subject line under 72 characters, lowercase, no period
- Use imperative mood ("add" not "added")
- Append ! after scope for breaking changes
```
