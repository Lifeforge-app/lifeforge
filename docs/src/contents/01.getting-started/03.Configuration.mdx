...
<Alert type="warning">
Due to the recent ongoing massive reworks towards LifeForge's codebase architecture and structure, a lot of scripts int he ForgeCLI are currently out-of-date. Those will only be updated when LifeForge reaches its next stable version.
</Alert>
...
<Alert type="tip">
Run `bun forge dev` to start the database, sever and client together.
</Alert>
</section>
