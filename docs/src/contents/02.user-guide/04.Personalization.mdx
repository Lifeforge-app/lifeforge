...
LifeForge comes with English language by default. However, you can add more languages to the system by installing language packs. Click the link below to learn how to add language packs to your system.

<CustomLink to="/user-guide/language-packs" text='Learn More' />

After installing the language packs, you can choose the language you prefer to use in the system by selecting the language from the dropdown list in the **Languages** section.
...
<Alert type="note">
  Feel free to click on the colors above to experience the power of LifeForge's
...
<Alert type="note">
  The background temperature preview above is interactive, so feel free to click on the color temperatures above to experience the power of LifeForge's theming capabilities.

  Also note that LifeForge now supports the four new grey tints introduced in Tailwind recently (`taupe`, `mauve`, `mist`, `olive`), which can be selected in the personalization page in your LifeForge instance, but they are not included in the preview above since I am lazy to take screenshot for the these new variants for now.

  We'll mark it as a `//TODO` for now :)
</Alert>
</section>
...
<Alert type="warning">
Due to the recent rework on the UI library, the support for semi transparent background for UI components are not yet in place.
</Alert>
...
<Alert type="note">
  If the Google Cloud API key is not set up in the system, the font family
  selector will be disabled, and you won't be able to change the font family.
  Learn more about how to set up the Google Cloud API key in the [API Keys
  Guide](/user-guide/api-keys).
</Alert>
...
![Font Scale](https://raw.githubusercontent.com/LifeForge-app/lifeforge-docs-media/main/assets/user-personalization/font-scale.webp)
</section>
